...
slides_inputs := (shellgrep−−no−filename′input/′(shell grep --no-filename 'input/' (slides_sections) slides.tex | grep -v '^%' | grep -o 'input/[A-Za-z0-9_\.\-]*\.[a-z]*')
...
slides.pdf: slides.tex (slidessections)(slides_sections) (slides_inputs)
	#module load texlive
	pdflatex -draftmode $<
	bibtex slides.aux
	pdflatex -draftmode $<
...
slides_notesonly.pdf: slides.tex (slidessections)(slides_sections) (slides_inputs)
